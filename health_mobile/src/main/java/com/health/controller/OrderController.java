...
public class OrderController {
	...
	public Result submit(@RequestBody Map map) {
		...
		// 从Redis中获取保存的验证码
		String validateCodeInRedis = jedisPool.getResource().get(
				telephone + RedisMessageConstant.SENDTYPE_ORDER);
		String validateCode = (String) map.get("validateCode");
		// 将用户输入的验证码和Redis中保存的验证码进行比对
		if (validateCodeInRedis != null && validateCode != null
				&& validateCode.equals(validateCodeInRedis)) {
			// 如果比对成功，调用服务完成预约业务处理
			...
			if (result.isFlag()) {
				// 预约成功，可以为用户发送短信
				try {
					// SMSUtils.sendShortMessage(SMSUtils.ORDER_NOTICE,telephone,
					// (String) map.get("orderDate"));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
